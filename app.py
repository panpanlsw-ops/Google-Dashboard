...
import calendar
from data import get_data, get_roi_data, get_regional_data, CAMPAIGNS
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab1, tab2 = st.tabs(["📈 Today", "🏢 Regional Offices"])
...
with tab2:
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px;white-space:nowrap;">
      ...
      <tfoot>
        <tr style="background:#f8f9fa;font-weight:600;border-top:0.5px solid #e5e7eb;">
    ...
    <script>
    const COLORS = {COLORS};
    const NAMES  = {pie_names};
    const LEADS  = {pie_leads};
    const SALES  = {pie_sales};
    const pieOpts = {{
    ...
    new Chart(document.getElementById('pie-leads'), {{
      type:'doughnut',
      data:{{ labels:NAMES, datasets:[{{ data:LEADS, backgroundColor:COLORS, borderWidth:1, borderColor:'#fff' }}] }},
      options: pieOpts
    }});
    new Chart(document.getElementById('pie-sales'), {{
      type:'doughnut',
      data:{{ labels:NAMES, datasets:[{{ data:SALES, backgroundColor:COLORS, borderWidth:1, borderColor:'#fff' }}] }},
      options: pieOpts
    }});
    </script>
    """

    st.components.v1.html(full_html, height=len(offices)*40 + 820, scrolling=False)
